...
def detect(candles: list[dict[str, Any]], timeframe: str = "5m") -> dict[str, Any]:
    ...
    is_invalidated = close > max(first["price"], second["price"]) + tolerance
    ...
    return build_result(
        ...
        reason="Two similar swing highs formed recently without invalidation; a neckline break confirms bearish pressure.",
        timeframe=timeframe,
        bars_used=len(clean),
    )

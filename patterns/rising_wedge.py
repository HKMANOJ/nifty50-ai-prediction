...
def detect(candles: list[dict[str, Any]], timeframe: str = "5m") -> dict[str, Any]:
    ...
    if len(highs) < 2 or len(lows) < 2:
        return empty_result("Rising Wedge", "reversal", timeframe, len(clean))
    ...
    detected = high_line[0] > 0 and low_line[0] > 0 and low_line[0] > high_line[0]
    breakdown = clean[-1]["close"] < lower
    confidence = 62 + (16 if breakdown else 0)
    return build_result(
        name="Rising Wedge",
        ...
        levels={"lower_trendline": lower, "breakdown": breakdown, "high_slope": high_line[0], "low_slope": low_line[0]},
        reason="Price is rising in a narrowing channel, which often warns of upside exhaustion.",
        timeframe=timeframe,
        bars_used=len(clean),
    )
